...
const imagePreview = ref('') // New ref for preview
const searchQuery = ref('')
const selectedCategoryFilter = ref('ทั้งหมด')
const showLowStockOnly = ref(false)
const newCategoryName = ref('')
const newCategoryInput = ref('') // ตัวแปรใหม่สำหรับรับค่าพิมพ์หมวดหมู่ใหม่ใน Modal
...
const openAddModal = () => {
  ...
  imagePreview.value = '' 
  newCategoryInput.value = '' // ล้างค่าหมวดหมู่ใหม่
  currentProduct.value = {
    name: '',
    category: categories.value.find(c => c !== 'ทั้งหมด') || 'อื่นๆ',
    price: 0,
    cost: 0,
    stock: 0,
    barcode: '',
...
const openEditModal = (product: Product) => {
  ...
  // เช็คว่าหมวดหมู่ของสินค้ามีอยู่ในรายการหรือไม่ ถ้าไม่มีให้เอามาใส่ช่องพิมพ์ใหม่
  const isExistingCategory = categories.value.includes(product.category)
  newCategoryInput.value = (!isExistingCategory && product.category) ? product.category : ''
              ...
              <div class="flex flex-col md:flex-row gap-10">
                <div class="w-full md:w-56 space-y-4">
                  ...
                  <div class="relative w-full aspect-square bg-slate-50 rounded-[2rem] overflow-hidden border-2 border-dashed border-slate-200 flex items-center justify-center group hover:bg-slate-100 hover:border-indigo-300 transition-all cursor-pointer">
                    <img v-if="imagePreview || currentProduct.image" :src="imagePreview || getImageUrl(currentProduct.image)" class="w-full h-full object-cover" />
                    ...
                    <div v-if="isUploadingImage" class="absolute inset-0 bg-slate-900/20 backdrop-blur-[2px] flex flex-col items-center justify-center transition-all">
                      <div class="w-8 h-8 border-4 border-white border-t-transparent rounded-full animate-spin mb-3"></div>
                      <span class="text-[10px] font-black text-white uppercase tracking-widest drop-shadow-md">กำลังอัปโหลด...</span>
                    </div>
                  ...
                  <button v-if="imagePreview || currentProduct.image" type="button" @click="currentProduct.image = ''; imagePreview = ''"
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-3">
                      ...
                      <div class="flex flex-col gap-3">
                        ...
                        <div class="relative">
                          <select v-model="currentProduct.category" @change="newCategoryInput = ''"
                            class="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl font-bold focus:ring-2 focus:ring-indigo-500 transition-all outline-none appearance-none cursor-pointer">
                            <option v-if="newCategoryInput && !categories.includes(newCategoryInput)" :value="newCategoryInput" class="hidden">
                              (หมวดหมู่ใหม่) {{ newCategoryInput }}
                            </option>
                            <option v-for="cat in categories.filter(c => c !== 'ทั้งหมด')" :key="cat" :value="cat">{{ cat }}</option>
                          </select>
                        ...
                        <div class="relative">
                          <input type="text" v-model="newCategoryInput" @input="currentProduct.category = newCategoryInput"
                            class="w-full px-6 py-4 bg-white border border-slate-200 rounded-2xl font-bold focus:ring-2 focus:ring-indigo-500 transition-all outline-none placeholder:font-normal placeholder:text-slate-400"
                ...
                <div class="space-y-3">
                  ...
                  <div v-if="selectedProductForStockIn" class="p-4 bg-indigo-50 rounded-2xl border border-indigo-100 flex items-center gap-4 animate-in fade-in zoom-in-95 duration-200">
                     <div class="w-12 h-12 bg-white rounded-xl flex items-center justify-center overflow-hidden flex-shrink-0 border border-indigo-100">
                        <img v-if="selectedProductForStockIn.image" :src="getImageUrl(selectedProductForStockIn.image)" class="w-full h-full object-cover" />
